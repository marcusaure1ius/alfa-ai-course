# ADR-0015: Индивидуальная n8n identity и отзыв через обязательный gateway

Дата: 2026-08-02
Статус: superseded частично ADR-0013

Нумерация: решение было принято под номером ADR-0012 и получило номер 0015
позже, когда выяснилось, что три разных решения одновременно заняли 0012.
Поэтому более ранний по номеру ADR-0013 отменяет часть этого документа —
хронологически он написан после него. Номера не переиспользовались.

ADR-0013 сохраняет обязательный revocable gateway и индивидуальную identity,
но заменяет ручной shared secret и предварительное приглашение автоматической
server-side настройкой.
