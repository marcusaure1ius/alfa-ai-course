...
- файл подтверждения владения в поисковой консоли больше не зашит в общий
  managed-профиль. Он привязан к аккаунту владельца, а не к сайту, а профиль
  едет в публичном релизном артефакте и разворачивается на произвольных хостах:
  любая посторонняя установка отдавала бы чужое доказательство владения. Имя
  файла приходит из `SITE_VERIFICATION_FILE` конкретного хоста, тело ответа
  выводится из него же (T-0123);
...
- Managed-конфиг изменён относительно закреплённого `v0.1.5`, поэтому
  `release_pin_test` красный до перевыпуска релиза и обновления pin.
