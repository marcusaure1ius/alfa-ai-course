...
стороне Vercel, пережившим удаление веток в GitHub, а при локальных ветках
(см. «Ветки и выкатка» в [AGENTS.md](../AGENTS.md)) git-превью и не
используются. Если ветка попадёт на `origin`, проверки Vercel у её PR не
будет вовсе. Ручное превью: `vercel` без `--prod` из корня репо.
...
сверяйте `meta.githubCommitSha` последнего production deployment с тем
коммитом, который выкатывали.

Это отвечает на вопрос «что в проде», но не на вопрос «выкатили ли нужное».
Второй строже и живёт в разделе «Ветки и выкатка» [AGENTS.md](../AGENTS.md):
деплой обязан содержать последнее изменение кода платформы, попавшее в `main`.
Совпадение с выкаченным коммитом само по себе не значит, что выкачено
актуальное — прод уже простаивал три дня на устаревшем коммите при зелёной
сверке.
