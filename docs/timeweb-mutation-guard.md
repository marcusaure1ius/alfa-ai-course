...
- Scope: typed mutation contract, guards и fake/reconciliation tests
- Реальный Timeweb token не подключался; VPS, IP и DNS не создавались
...
Create body строится внутри adapter только из typed полей `name`, `preset_id` и
`os_id`. Update body содержит только `name`. `server_id` обязан быть
...
находит active ownership record и завершает step без второго create-вызова.
...
token. `TIMEWEB_API_TOKEN` не добавляется в `.env.example`, browser DTO,
PostgreSQL, logs или audit.
...
Этот документ и тесты не являются evidence реального Timeweb account, token
permissions, VPS, списания денег или успешного удаления provider resource.
