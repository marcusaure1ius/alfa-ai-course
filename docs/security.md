...
параметры по именам: в логе остаётся только путь. Так же обрабатываются `Referer`
и `Location` — первый несёт полный URL предыдущей страницы на same-origin
подзапросы, второй может нести код в редиректе от n8n. Отдельно удаляются
заголовки `Cookie`, `X-N8n-Api-Key` и `X-Neurokurs-Management`.

Диагностическая цена признана приемлемой: несекретные параметры вроде `state`
тоже перестают попадать в лог. Путь, метод, код ответа и время остаются.
...
**каждого** логгера обязаны стоять `regexp`-фильтры на `request>uri`, `Referer`
и `Location` плюс удаление трёх заголовков. Вторая — отсутствие
`log_credentials`. Третья — **живой запрос**: тринадцать подложенных секретов в
query, заголовках и `Referer`, ни один не должен попасть в лог.
