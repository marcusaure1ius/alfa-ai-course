...
Через общий `handle` идут два запроса, в query которых лежит секрет:
...
- `/rest/oauth2-credential/callback?code=...&state=...` — OAuth2-код n8n. Он
  обменивается на токен доступа к внешнему сервису ученика.

Тот же `token` приезжает и **заголовком** `Referer`: при нашей
`Referrer-Policy: strict-origin-when-cross-origin` браузер шлёт полный URL
предыдущей страницы на same-origin подзапросы, а n8n — одностраничное
приложение, поэтому `/signup?token=...` попадает в лог со следующим же запросом
за скриптом или стилем.

`config/Caddyfile.platform` редактирует `token` и `code` и в `request>uri`, и в
`Referer`, а также удаляет заголовки `Cookie`, `X-N8n-Api-Key` и
`X-Neurokurs-Management`. Соседний `state` **не** редактируется намеренно: это
CSRF-nonce, он мало что даёт без `code`, а по нему сопоставляют запрос и ответ
при разборе неудачной привязки credential.

Здесь четыре ловушки, и все четыре уже срабатывали:
...
- **имя заголовка сравнивается с канонической формой Go.** Написанное as-is
  ...
  no-op, а ключ управления печатается в лог целиком. Правильная форма —
  `X-N8n-Api-Key`. Найдено независимым ревью T-0118 и воспроизведено живым
  запросом: `Cookie` и `X-Neurokurs-Management` удалялись, потому что их
  написание уже каноническое, а ключ утекал;
- гейт легко написать так, что он считает логгеры **с** редакцией вместо того,
  ...
  логгеров, поэтому добавление третьего — `log extra { output stdout }` —
  оставляло её зелёной при живом логгере без редакции. Воспроизведено (T-0118).

Гейт `compose-config` поэтому проверяет две разные вещи. Первая — результат
`caddy adapt`: у **каждого** логгера из `.logging.logs` обязаны быть замены
`token` и `code` в `request>uri` и в `Referer` плюс удаление трёх заголовков.
Вторая — **живой запрос** с подложенными секретами: в лог не должен попасть ни
один из них.

Вторая проверка добавлена не для полноты. Первая смотрит на объявленные поля и
поэтому пропускала ровно ту ловушку с каноническим именем: директива есть, а
эффекта нет. Живой прогон дополнительно проверяет, что лог вообще виден —
иначе «ноль секретов» означал бы только то, что читать было нечего.

Из этого следует правило для правок профиля: набор редактируемых полей у обоих
логгеров обязан совпадать. Расходятся — гейт краснеет, и это не придирка:
default-логгер и access-логгер видят разные запросы.
