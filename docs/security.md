...
`config/Caddyfile.platform` редактирует оба параметра и удаляет `Cookie`,
`X-N8N-API-KEY` и `X-Neurokurs-Management` из логов. Соседний `state` **не**
редактируется намеренно: это CSRF-nonce, без `code` он бесполезен, а по нему
сопоставляют запрос и ответ при разборе неудачной привязки credential.

Здесь три ловушки, и все три уже срабатывали:
...
- гейт легко написать так, что он считает логгеры **с** редакцией вместо того,
  ...
  оставляло её зелёной, а живой запрос печатал сырой токен в
  `http.log.access.extra` и `http.log.error.extra`. Воспроизведено (T-0118).

Поэтому гейт `compose-config` проверяет результат `caddy adapt`, а не текст
конфига, и требует у **каждого** логгера из `.logging.logs` замены `token` и
`code` плюс удаления трёх заголовков. Новый логгер без редакции роняет проверку.
