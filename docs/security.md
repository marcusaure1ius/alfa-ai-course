...
все заголовки запроса, включая `User-Agent`, в лог не попадают.

Что **остаётся** в записи, снято живым запросом, а не по памяти:
`remote_ip`, `remote_port`, `client_ip`, `proto`, `method`, `host`, `uri` с
вырезанным query, `bytes_read`, `user_id`, `duration`, `size`, `status` и
**целиком карта `resp_headers`**.
...
Из этого следуют два остаточных канала, и оба названы здесь намеренно:

- **`resp_headers` не фильтруется.** `Set-Cookie` Caddy редактирует сам,
  `Location` закрыт `regexp`, а произвольный заголовок от n8n печатается сырым —
  воспроизведено с фиктивным upstream. Это открытый канал, вынесенный в
  отдельную задачу: удалять все поля ответа нельзя не думая, это оставит без
  диагностики проксирования;
...
- **`servers { log_credentials }` снимает встроенную редакцию Caddy** и не виден
  ни в одном фильтре полей: опция живёт вне логгера. Под нынешним профилем
  `Authorization` она не вернёт — заголовки запроса удаляются целиком, — но
  `Set-Cookie` начнёт печататься сырым, а это сессионный JWT n8n. Проверено
  живым запросом.
