...
**Ответные заголовки удаляются целиком** (T-0127) — той же логикой, что и
запросные: их имена задают n8n и сам ученик (нода Respond to Webhook принимает
произвольные заголовки ответа), поэтому любой точечный фильтр неполон по
построению. Прежний профиль закрывал только `Location`.

Размен решения записан явно. Что давала каждая категория ответных заголовков и
чем это заменимо:

- `Location` — диагностика редиректов и OAuth-цепочек. Заменимо: код `3xx`
  остаётся в записи, цель same-host видна следующим запросом в том же логе,
  внешняя цель воспроизводится `curl -sI` с хоста;
- `Content-Type` — различение HTML-error-body и API-ответа. Заменимо тем же
  `curl`;
- security-заголовки (`Strict-Transport-Security`, `X-Frame-Options`, …) —
  задаются самим Caddy в этом же конфиге, логировать собственную константу
  незачем;
- `Set-Cookie` — и так печатался как `REDACTED`;
- «upstream ответил или нет» — держится на `status` и на error-логгере
  `reverse_proxy`, который пишет текст ошибки, а не заголовки.

Вариант «оставить только `Location`» отвергнут не вкусом, а фактом: в stock
Caddy нет фильтра «удалить всё, кроме», а сочетание удаления карты с точечным
фильтром по её полю оставляет точечный фильтр мёртвым при любом порядке
применения — проверено адаптацией и живым запросом на закреплённом образе.

`Referer` отдельного фильтра не требует — он входит в удаляемую карту
заголовков запроса.

Что **остаётся** в записи, снято живым запросом с фиктивным upstream, а не по
памяти: `remote_ip`, `remote_port`, `client_ip`, `proto`, `method`, `host`,
`uri` с вырезанным query, `bytes_read`, `user_id`, `duration`, `size`,
`status`. Карты `resp_headers` в записи больше нет.

Под TLS добавляется блок `request>tls`: `resumed`, `version`, `cipher_suite`,
`proto`, `server_name`, `ech`. На боевом хосте он есть всегда — `N8N_HOST` домен,
и n8n получает `https`-URL. Секретов в блоке нет, но опись без него неполна.

Остаточный канал остаётся один, и он назван намеренно:

- **путь остаётся в логе намеренно** — на нём держится диагностика. Если вебхук
  настроен без `headerAuth`, секретом служит сам путь, и он попадёт в лог.
  Workflow репозитория используют `headerAuth`, но ученик может настроить иначе.
...
- **`servers { log_credentials }` снимает встроенную редакцию Caddy** и не виден
  ни в одном фильтре полей: опция живёт вне логгера. Под нынешним профилем она
  не возвращает ничего — обе карты заголовков удаляются целиком, что проверено
  живым запросом с включённой опцией (T-0127), — но опасна в сочетании с любым
  будущим логгером без вырезания, поэтому гейт запрещает её отдельно.

Гейт `compose-config` проверяет три вещи. Первая — результат `caddy adapt`: у
**каждого** логгера обязаны стоять `regexp`-фильтр на `request>uri` и удаление
обеих карт заголовков — `request>headers` и `resp_headers`. Вторая — отсутствие
`log_credentials`. Третья — **живой запрос**: двадцать подложенных секретов в
query, в заголовках запроса с заведомо неизвестными именами, в `Referer`, а с
фиктивным upstream — и в ответных заголовках, cookie и `Location` редиректа; ни
один не должен попасть в лог.
...
остаётся в логе намеренно, — и что фиктивный upstream действительно ответил
через прокси: иначе «ноль секретов» означал бы только то, что читать было
нечего или ответные планты не испускались.
