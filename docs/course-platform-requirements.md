...
`N8N-09` Пространство ученика показывает адрес инструмента и состояние его
аккаунта в нём. Пока аккаунт не активирован, показывается шаг «задайте пароль»
со ссылкой на одноразовое приглашение n8n; после активации ссылка стирается и
остаётся адрес инструмента. Пароль ученика платформа не генерирует, не хранит,
не показывает и не пишет в логи: Public API n8n пароли не принимает.
