 ...
 *
 * `process.pid` здесь берётся при чтении конфигурации, то есть в главном
 * процессе vitest; воркеры получают уже готовый путь через provide/inject
 * плагина. Импортировать этот модуль из кода воркера нельзя — там pid
 * другой, и путь молча разойдётся с каталогом прогона.
 */
