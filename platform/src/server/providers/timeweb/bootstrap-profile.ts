...
export const STARTER_KIT_BOOTSTRAP_PROFILE = Object.freeze({
  ...
  version: "starter-kit-v0.1.15",
  // v0.1.13 вырезал из лога Caddy ответные заголовки целиком (T-0127): их
  // имена задают n8n и сам ученик, поэтому точечные фильтры неполны; это же
  // первый релиз с путём обновления кода развёрнутых хостов (T-0130).
  // v0.1.12 перевыпущен из-за неверной описи полей лога, v0.1.11 распространил
  // вырезание на заголовки запроса, v0.1.10 сделал то же для query (T-0118).
  ...
  installerUrl:
    ...
    "alfa-ai-course/releases/download/v0.1.13/install.sh",
  installerSha256:
