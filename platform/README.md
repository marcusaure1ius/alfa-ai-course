...
Платформа показывает адрес инструмента и состояние аккаунта, но не проксирует
вход: ticket, gateway cookie и `forward_auth` удалены. Логин ученика — его адрес
почты в Neurokurs, пароль он задаёт сам по одноразовому приглашению. Отсюда важное следствие —
...
5. Ученик открывает страницу инструмента в своём пространстве. Пока n8n считает
   аккаунт pending, там показано состояние «задайте пароль» и кнопка на
   одноразовое приглашение; после его принятия остаётся обычный адрес
   инструмента. Ни admin, ни почта в этом шаге не участвуют: SMTP в
   managed-профиле не настроен, а Public API n8n пароли не принимает.
6. При revoke/expiry account и учебные данные не удаляются автоматически.
   Одноразовые данные приглашения удаляются сразу при revoke, при первом
   обновлении состояния после активации аккаунта либо ежедневным
...
<!-- T-0122: проба после отключения git-деплоев; PR закрывается без мержа -->
