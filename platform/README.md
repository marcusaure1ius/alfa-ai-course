...
`/admin` открывает фактический обзор: точки внимания, последние изменения и
переходы к ученикам, контенту и инструментам. Счётчики строятся из PostgreSQL;
если данных нет, интерфейс не подставляет демонстрационные KPI.
